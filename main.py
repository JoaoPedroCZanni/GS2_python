...
def inicializar_banco():
    ...
    if conn:
        try:
            ...
            """)
            cursor.execute("SELECT table_name FROM user_tables WHERE table_name = 'USUARIOS'")
...
def cadastrar_usuario_banco(cpf, email, senha):
    ...
    if conn:
        try:
            print(f"Cadastrando usuário: {email}")  # Log para verificação
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO usuarios (cpf, email, senha) VALUES (:1, :2, :3)
            """, (cpf, email, senha))
            conn.commit()
...
def listar_pessoal():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT cpf, email FROM usuarios")
            usuarios = cursor.fetchall()
            if usuarios:
                for i, (cpf, email) in enumerate(usuarios, 1):
                    print(f"{i}. CPF: {cpf}, E-mail: {email}")
            else:
                print(MSG_NENHUM_CADASTRO)
        except oracledb.DatabaseError as e:
            print("Erro ao listar cadastros:", e)
        finally:
            conn.close()
    input("\nPressione Enter para voltar ao menu...")
...
def adicionar_cadastro_entrada():
    ...
    if not validar_senha(senha):
        ...
        input("\nPressione Enter para voltar ao menu...")
        return
    cadastrar_usuario_banco(cpf, email, senha)
    input("\nPressione Enter para voltar ao menu...")
